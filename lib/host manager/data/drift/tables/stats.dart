...
import 'package:cipherly/host%20manager/data/models/host_stats.dart';
...
class StatsDao extends DatabaseAccessor<DB> with _$StatsDaoMixin {
  ...
  Future addHost(DriftHost host) =>
      into(hostsTable).insert(host, mode: InsertMode.insertOrReplace);
  Future updateHost(DriftHost host) => update(hostsTable).replace(host);
  Future deleteHost(String host) =>
      (delete(hostsTable)..where((t) => t.adress.equals(host))).go();
  Future deleteAllHosts() => delete(hostsTable).go();
  Future<List<DriftHost>> getAllHosts() => select(hostsTable).get();

  /// Pings and stats

  Future wipeExpiredPings(DateTime before) => (delete(pingTable)
        ..where((t) =>
  ...
  Future<List<DriftPing>> hostPingsPeriod(
      String host, DateTime from, DateTime to) async {
  ...
  Future<List<DriftPing>> hostPingsPeriodScaled(
      String host, DateTime from, DateTime to, int scale) async {
    final fromStamp = from.millisecondsSinceEpoch;
    final toStamp = to.millisecondsSinceEpoch;
  ...
  Future<HostStats> hostStats(String host) async {
    ...
    return HostStats(avg.truncate(), min.truncate(), max.truncate(), count,
        lostCount, lossPercent);
  }
}
...
class HostsTable extends Table {
  ...
  TextColumn get adress =>
      text().unique().withLength(min: 3).withLength(min: 1)();
...
class PingTable extends Table {
  ...
  TextColumn get host =>
      text().references(HostsTable, #adress, onDelete: KeyAction.cascade)();
