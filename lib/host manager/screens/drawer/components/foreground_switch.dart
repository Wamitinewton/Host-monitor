...
class _ForegroundSwitchState extends State<ForegroundSwitch> {
  ...
  void initState() {
    ...
    settingsRepository.getSettings
        .then((value) => setState(() => _foreground = value.andWakeLock));
  }

  bool get foreground => _foreground;
  set foreground(bool v) {
    setState(() => _foreground = v);
    debouncer(() => settingsRepository.getSettings.then((settings) =>
        settingsRepository.setSettings(settings.copyWith(andForeground: v))));
  }
    ...
    return SizedBox(
      ...
      child: Row(
        children: [
          ...
          const Tooltip(
            message:
                'Foreground service its an android mechanism to prevent app from being killed by system by showing a notification.',
