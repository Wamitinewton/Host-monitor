...
import '../../../data/models/host_stats.dart';
...
class _TileHostStatsState extends State<TileHostStats> {
  ...
  Widget build(BuildContext context) {
    const tstyle = TextStyle(
        color: Color(0xffF5F5F5), fontSize: 12, fontWeight: FontWeight.w400);

    if (stats == null) return const Text('Loading...', style: tstyle);
    return Text(
        'AVG: ${stats!.avg} ms  MIN: ${stats!.min} ms  MAX: ${stats!.max} ms  LOSS: ${stats!.lossPercent}% COUNT: ${stats!.count}s',
        style: tstyle);
  }
}
