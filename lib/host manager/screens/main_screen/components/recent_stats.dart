...
class _RecentStatsState extends State<RecentStats> {
  ...
  int get min => samplesQueue.fold(
      5000, (min, ping) => ping.latency < min ? ping.latency : min);
  ...
  init() async {
    ...
    Future.doWhile(() async {
      final event = await statsRepository.eventBus
          .firstWhere((event) => event is PingAdded && event.ping.host == host);
  ...
  Widget build(BuildContext context) {
    TextStyle st = const TextStyle(fontSize: 8, fontWeight: FontWeight.w400);
    return Column(
      ...
      children: [
        ...
        Text('LOS: ${lossPercent.clamp(0, 100).toStringAsFixed(2)} %',
            style: st),
      ],
    );
  }
