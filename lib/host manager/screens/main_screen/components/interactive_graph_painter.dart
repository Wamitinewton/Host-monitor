...
import 'package:cipherly/host%20manager/core/formatters.dart';
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double i = 0; i < 0.99; i += (1 / (scale.floor() * 10))) {
      if (viewPortStartPercent < i && viewPortEndPercent > i) {
        ...
        String timeText = '';
        if (viewTimeDiff < const Duration(minutes: 2)) {
          timeText = time.numhms;
        } else if (viewTimeDiff < const Duration(days: 2)) {
          timeText = time.numhm;
        } else {
          timeText = time.numdm;
        }

        TextSpan span = TextSpan(style: const TextStyle(color: Color(0xffF5F5F5), fontSize: 8, fontWeight: FontWeight.w200), text: timeText);
        TextPainter tp = TextPainter(text: span, textAlign: TextAlign.center, textDirection: TextDirection.ltr);
        tp.layout();
